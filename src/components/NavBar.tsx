...
const navItem = tv({
	base: "hover:text-(color:--primary-color) hover:ease-in-out hover:transition-colors hover:duration-300 font-bold text-[80%] cursor-pointer",
	variants: {
		active: {
			true: "text-(color:--primary-color)",
		}
	}
})
	...
	return (
		<div className="m-0 flex justify-center items-center">
			<nav className="flex justify-center items-center fixed bottom-6 md:bottom-auto md:top-6 z-[30]">
				<div className="bg-(color:--content-color) text-(color:--secondary-color) rounded-3xl pl-0 pr-0 w-xs h-10 shadow-[0_2px_10px_rgba(0,0,0,0.1)]">
					<ul className="flex pl-6 pr-6 w-full h-full justify-between items-center">
						...
						<li onClick={() => applyTheme(theme === "light" ? "dark" : "light")} className="cursor-pointer">
